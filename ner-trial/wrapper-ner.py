...
class AdversarialTrainingArguments(TrainingArguments):
    def __init__(self, epsilon, alpha=0.5, gamma=0, ner=False, *args, **kwargs):
...
class AdversarialTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        #     labels = None
        if self.args.ner:
            ner_scores = inputs.pop('ner_scores')
        else: 
            ner_scores = None
        outputs = model(**inputs)
        if self.args.ner:
            inputs['ner_scores'] = ner_scores
...
def binary_focal_loss(logits, true_labels, alpha=0.5, gamma=0, ner_scores=None):
    pos_idx = torch.Tensor(true_labels == 1).long()
    neg_idx = torch.Tensor(true_labels == 0).long()
    pred_probs = torch.nn.Softmax(dim=1)(logits)[:, 1]

    if not torch.any(ner_scores):
        ner_scores = torch.zeros(size=true_labels.size()).long().cuda()

    loss_pos = - alpha * torch.sum(((1 - pred_probs)**(gamma*(1-ner_scores)**5) * torch.log(pred_probs)) * pos_idx)
    loss_neg = - (1-alpha) * torch.sum((pred_probs**(gamma*(1-ner_scores)**5) * torch.log(1-pred_probs)) * neg_idx)
